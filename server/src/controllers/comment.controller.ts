...
const addCommentOnMovie = dbHandler(async (req, res) => {
	...
	const { text, email, name, commentetor_avatar } = req.body;

	if (!isValidObjectId(id) || !text || !email || !name || !commentetor_avatar)
		return res
			.status(400)
	...
	if (!comment)
		return res
			.status(500)
			.json(new ApiError(500, "Failed to create comment"));
	...
	res.status(201).json(
		new ApiResponse(201, comment, "Comment added sucessfully")
	);
});
...
const updatedCommentById = dbHandler(async (req, res) => {
	...
	if (!comment)
		return res
			.status(500)
			.json(new ApiError(500, "Failed to update comment"));
...
const getCommentsByMovieId = dbHandler(async (req, res) => {
	...
	if (!comments)
		return res
			.status(500)
			.json(new ApiError(500, "Failed to fetch comments"));
...
const deleteCommentById = dbHandler(async (req, res) => {
	...
	if (!comment)
		return res
			.status(500)
			.json(new ApiError(500, "Failed to delete comment"));
	...
	res.status(200).json(
		new ApiResponse(200, comment, "Comment deleted sucessfully")
	);
});
